...
texts, target = text_helper.load_movie_data()
texts = text_helper.normalize_text(texts, stops)
target = [target[ix] for ix, x in enumerate(texts) if len(x.split()) > 2]
texts = [x for x in texts if len(x.split()) > 2]

word_dictionary = text_helper.build_dictionary(texts, vocabulary_size)
word_dictionary_rev = dict(zip(word_dictionary.values(), word_dictionary.keys()))
text_data = text_helper.text_to_numbers(texts, word_dictionary)
...
for i in range(generations):
    batch_inputs, batch_labels = text_helper.generate_batch_data(text_data, batch_size, window_size, method='cbow')
...
loss_x_vec = []
for i in range(generations):
    batch_inputs, batch_labels = text_helper.generate_batch_data(text_data, batch_size,
