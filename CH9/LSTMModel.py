...
class LSTM_Model():
    def __init__(self, embedding_size, rnn_size, batch_size, learning_rate, training_seq_len, vocab_size, infer_sample=False):
        ...
        self.modle_output = tf.nn.softmax(self.logit_output)
        ...
        self.cost = tf.reduce_mean(loss) / (self.batch_size * self.training_seq_len)
    ...
    def sample(self, sess, words=ix2vocab, vocab=vocab2ix, num=10, prime_text='thou art'):
        ...
        for n in range(num):
            ...
            x[0, 0] = vocab[word]
            feed_dict = {self.x_data: x, self.initial_state: state}
            [model_output, state] = sess.run([self.modle_output, self.final_state], feed_dict=feed_dict)
    ...
    for ix, batch in enumerate(batches):
        ...
        if iteration_count % eval_every == 0:
            for sample in prime_texts:
                print(test_lstm_model.sample(sess, ix2vocab, vocab2ix, num=10, prime_texts=sample))
